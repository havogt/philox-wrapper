 ...
 * The benefit of CBRNGs is the absence of large states which are costly to read
 * and write in CUDA code.
 * For a detailed discussion see "Parallel random numbers: as easy as 1, 2, 3"
 * (http://dl.acm.org/citation.cfm?doid=2063405)
 *
 * Any combination of the 4 * 32-bit counter and 2 * 32-bit key gives a
 * uncorrelated random number.
 * As key we use the combination (thread id, seed), where seed can be chosen by
 * the user.
 * The counter is (kernelCounter, globalCounter, *, * ) where
 *  - kernelCounter is a local variable of the kernel and is incremented by each
 * call to generate new random numbers
 *  - globalCounter is a kernel parameter, incrementing has to be done on host
 * side. (TAKE CARE TO DO IT!)
 *    This means that each kernel that uses random numbers has to have an
 * counter parameter which has to be incremented on each kernel call.
 ...
 *
 * The static getNextCounter() function gives a global (runtime-wide) counter
 * which can be given to the constructor.
 * I don't want to do this implicitly to avoid mixing of host and device
 * variables.
 */
...
template <typename T> class PhiloxWrapper {
  public:
    __device__ inline PhiloxWrapper(int tid, int seed,
  ...
  private:
    ...
    union {
        ...
        typename PhiloxWrapperInfo<T>::uintType
            i[PhiloxWrapperInfo<T>::uintSize];
    } u;
    short localCounter;
...
template <typename T>
__device__ PhiloxWrapper<T>::PhiloxWrapper(int tid, int seed,
                                           unsigned int globalCounter) {
    k[0] = tid;
    k[1] = seed;
    c[0] = 0; // kernelCounter
...
/**
 * Tested in Cuda5.5: This method uses no stack frame (compared to 52 bytes of
 * rand()) but 30 registers (rand() can be forced to use 23 registers with
 * maxrregcount otherwise it will use 31 but with better performance).
 * Still it is slower. Maybe for some applications this method will be better.
 * (Maybe in case of register pressure?)
 *
 * I also tried a variant which generates a new set after every 2 random numbers
 * (instead of 4 for float). As expect, this is also slightly slower. The idea
 * was inspired by the (comparably) very fast double precision rngs!
...
template <>
__device__ inline double PhiloxWrapper<double>::randExp(double lambda) {
    if (localCounter == 0) // we have to calculate two new doubles
    {
        localCounter = 1;
        c[0]++; // inkrement the kernel counter
...
template <>
__device__ inline float PhiloxWrapper<float>::randExp(float lambda) {
    if (localCounter == 0) // we have to calculate 4 new floats
    {
        localCounter = 3;
        c[0]++; // inkrement the kernel counter
...
template <> __device__ inline float2 PhiloxWrapper<float>::randNormal() {
    ...
    } else // we have another float available.
    {
        localCounter--;
        double temp =
            ::sqrt(-2. * ::log(u01_open_open_32_24(u.i[localCounter])));
        localCounter--;
...
/**
 * Use this to get a global (static) counter to initialize the kernel-specific
 * PhiloxWrapper.
 */
...
template <typename T>
__host__ unsigned int PhiloxWrapper<T>::getCurrentCounter() {
    return globalCounter;
}
}
